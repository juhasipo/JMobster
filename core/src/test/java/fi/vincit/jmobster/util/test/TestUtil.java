...
public class TestUtil {
    ...
    public static int assertFieldFoundOnce( List<ModelField> modelFields, String fieldName ) {
        ...
        for( int i = 0; i < modelFields.size(); ++i ) {
            ...
            if( field.getName().equals(fieldName) ) {
                if( foundIndex >= 0 ) {
                    Assert.assertTrue("Field <" + fieldName + "> found more than once.", false);
                }
                foundIndex = i;
            }
        }
        if( foundIndex < 0 ) {
            Assert.assertTrue("Field with name <" + fieldName + "> not found.", false);
    ...
    public static void assertFieldNotFound(List<ModelField> modelFields, String fieldName) {
        for( ModelField field : modelFields ) {
            if( field.getName().equals(fieldName) ) {
                Assert.assertTrue("Field with name <" + fieldName + "> found when it should be ignored.", false);
            }
        }
    }
